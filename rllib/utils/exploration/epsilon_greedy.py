...
class EpsilonGreedy(Exploration):
    ...
    def __init__(self, action_space: gym.spaces.Space, *, framework: str, initial_epsilon: float = 1.0, final_epsilon: float = 0.05,
            warmup_timesteps: int = 0, epsilon_timesteps: int = int(1e5), epsilon_schedule: Optional[Schedule] = None, **kwargs, ):
        ...
        self.epsilon_schedule = from_config(Schedule, epsilon_schedule, framework=framework) or PiecewiseSchedule(
            endpoints=[(0, initial_epsilon), (warmup_timesteps, initial_epsilon), (warmup_timesteps + epsilon_timesteps, final_epsilon), ],
            outside_value=final_epsilon, framework=self.framework, )

        # The current timestep value (tf-var or python int).
        self.last_timestep = get_variable(np.array(0, np.int64), framework=framework, tf_name="timestep", dtype=np.int64, )
    ...
    def get_exploration_action(self, *, action_distribution: ActionDistribution, timestep: Union[int, TensorType],
    ...
    def _get_tf_exploration_action_op(self, action_distribution: ActionDistribution, explore: Union[bool, TensorType],
        ...
        Returns:
            The tf exploration-action op. (action, logp)
        """
        # TODO: Support MultiActionDistr for tf.
        q_values = action_distribution.inputs   # only used for shape, so we don't need to bother with updated logits
        ...
        chose_random = (tf.random.uniform(tf.stack([batch_size]), minval=0, maxval=1, dtype=tf.float32) < epsilon)

        action = tf.cond(pred=tf.constant(explore, dtype=tf.bool) if isinstance(explore, bool) else explore,
            true_fn=(lambda: tf.where(chose_random, random_actions, exploit_action)), false_fn=lambda: exploit_action, )
    ...
    def _get_torch_exploration_action(self, action_distribution: ActionDistribution, explore: bool,
        ...
        if explore:
            ...
            if isinstance(action_distribution, TorchMultiActionDistribution):
                ...
                random_valid_action_logits = torch.where(q_values <= FLOAT_MIN, torch.ones_like(q_values) * 0.0, torch.ones_like(q_values), )
                # A random action.
                random_actions = torch.squeeze(torch.multinomial(random_valid_action_logits, 1), axis=1)
    ...
    def get_state(self, sess: Optional["tf.Session"] = None):
        ...
        return {"cur_epsilon": convert_to_numpy(eps) if self.framework != "tf" else eps,
            "last_timestep": convert_to_numpy(self.last_timestep) if self.framework != "tf" else self.last_timestep, }
